...
class FrankaPanda(Articulation):
    ...
    def differential_inverse_kinematics(
        ...
        q_wp = quaternion_multiplication(goal_quat_wp, current_quat_conjugate_wp)  # Resulting quaternion difference
    ...
    def apply_joint_action(self, joint_positions: np.ndarray, gripper_state: float) -> None:
        ...
        Args:
            ...
            gripper_state: Gripper position where 0.0 is closed and 1.0 is open.
                Will be normalized to [0.04, 0.0] range for each gripper finger.
        """
        ...
        # FrankaPanda has 9 DOFs: 7 arm + 2 gripper fingers
        full_positions = np.concatenate([
            joint_positions[:, :7],  # 7 arm joints
            np.full((joint_positions.shape[0], 2), gripper_position)  # 2 gripper fingers
        ], axis=1)
